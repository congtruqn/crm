...
const CreateCustomerEvaluation: React.FC<MyComponentProps> = ({ id, onSubmitSuccess, onCancel }: MyComponentProps)=>{  
  const { reset, register, handleSubmit, setValue,  formState: { errors } } = useForm();
  const onSubmit = async (data : unknown) => {
    if(id){
      const rep = await apiClient.put('/customer-evaluation/'+id, data);
      if(rep.status == 200){
        reset();
      }
    }
    else{
        const rep = await apiClient.post('customer-evaluation', data);
        if(rep.status == 200){
          reset();
        }
    }
  ...
  const getData = async (id: string): Promise<Customers | undefined> => {
    try {
        const response = await apiClient.get('/customer-evaluation/'+id); // Replace with your actual API endpoin
      ...
      <div className="row">
        ...
        <div className="form-group col-sm-12">
          <label className="col-sm-12 control-label">
            Tên đánh giá khách hàng
          </label>
          <div className="col-sm-12">
            <input {...register('name', { required: true })} className="form-control"/>
            {errors.name && <span className="error">Tên đánh giá là bắt buộc</span>}
          </div>
        </div>
...
export default CreateCustomerEvaluation;