...
const CreateCustomerStatus: React.FC<MyComponentProps> = ({ id, onSubmitSuccess, onCancel }: MyComponentProps)=>{  
  const { reset, register, handleSubmit, setValue,  formState: { errors } } = useForm();
  const onSubmit = async (data : unknown) => {
    if(id){
      const rep = await apiClient.put('/customer-status/'+id, data);
      if(rep.status == 200){
        reset();
      }
    }
    else{
        const rep = await apiClient.post('customer-status', data);
        if(rep.status == 200){
          reset();
        }
    }
  ...
  const getData = async (id: string): Promise<Customers | undefined> => {
    try {
        const response = await apiClient.get('/customer-status/'+id); // Replace with your actual API endpoin
      ...
      <div className="row">
        ...
        <div className="form-group col-sm-12">
          ...
          <div className="col-sm-12">
            ...
            {errors.name && <span className="error">Tên trạng thái là bắt buộc</span>}
          </div>
        </div>
