...
import CreateCustomerEvaluation from "../components/customer/createCustomerEvaluation";
...
const CustomerEvaluation: React.FC = ()=>{  
 ...
 const comfirmDelete = async () => {
    const response = await apiClient.delete('/customer-evaluation/'+customerId);
  ...
  const fetchData = async (pageSize: number, pageNumber: number) => {
    try {
        const response = await apiClient.get('/customer-evaluations?pageSize='+pageSize+'&pageNumber='+pageNumber); // Replace with your actual API endpoin
  ...
  return (
    <section>
      <h2 className="title">{"Danh sách đánh giá"}</h2>
      ...
      <Drawer
          ...
          open={open}
        >
          <CreateCustomerEvaluation id={customerId} onCancel={handleCloseModal} onSubmitSuccess={handleFormSubmit}/>
      </Drawer>
      <Modal
        title=""
        open={openModal}
